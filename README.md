...
- [ ] Thay auth `X-User-ID` bằng OTP/JWT (toàn bộ nằm ở `web/src/api.js` + hook `authUserID`)
- [ ] ZaloPay merchant credential thật (thay `zaloPayStub` trong `cmd/server`)
- [ ] `TOKEN_CIPHER_KEY` từ secret manager; cân nhắc envelope encryption KMS
- [ ] App Attest / Play Integrity verifier thật (interface `AttestationVerifier`)
- [ ] Partition bảng `activities` theo tháng
- [ ] Tắt `DEV_MODE`
